/**
 * searchBible.ts
 * Keyword search across all loaded Bible translations with scope filtering.
 * Scopes: 'bible' (whole), 'OT', 'NT', 'book' (one book), 'chapter' (one chapter).
 */

import { getBible, getRegisteredTranslations } from '../data/bibleLoader';
...
const OT_BOOKS = new Set(canonicalBooks.filter((b) => b.testament === 'OT').map((b) => b.name));
const NT_BOOKS = new Set(canonicalBooks.filter((b) => b.testament === 'NT').map((b) => b.name));
...
/**
 * Search for `query` across ALL registered translations + scope.
 * Matching is case-insensitive whole-substring (not whole-word).
 * Returns up to maxResults hits in canonical Bible order, grouped by translation.
 */
export function searchBible(query: string, options: SearchOptions): SearchResult[] {
  const { scope, scopeBook, scopeChapter, maxResults = 500 } = options;
  ...
  // Search all registered translations, not just the active one
  const allTranslations = getRegisteredTranslations();

  for (const trans of allTranslations) {
    const bible = getBible(trans);
